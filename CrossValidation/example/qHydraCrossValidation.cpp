...
public:
  ...
  ArrayMatrix score_matrix_;
  qHydra(){};
  qHydra(float first_lambda,float second_lambda,float third_lambda){
...
class qHydraCrossValidation:public Remover, public GeneSelection,public SplitStep{
protected:
...
public:
  ...
  qHydra qHydra_;
  qHydraCrossValidation(){};
  virtual ~qHydraCrossValidation(){};
  ...
  void Preserve(){
    preserve_third_ = qHydra_.third_.bipartite_graph_.array_matrix_;
    preserve_second_ = qHydra_.second_.bipartite_graph_.array_matrix_;
  }
  ...
  virtual void Process(ROCScore &roc,int i){
    ...
    qHydra_.second_.bipartite_graph_.array_matrix_ = preserve_second_;
    qHydra_.third_.bipartite_graph_.array_matrix_ = preserve_third_;
    int N = qHydra_.third_.bipartite_graph_.array_matrix_.dims(0);
    int L = qHydra_.second_.bipartite_graph_.array_matrix_.dims(1);
    gfor(af::array a_i,N){
      qHydra_.third_.bipartite_graph_.array_matrix_(a_i,position) = 0;
    }
    gfor(af::array a_j,L){
      qHydra_.second_.bipartite_graph_.array_matrix_(position,a_j) = 0;
    }
    RemoveEmptyCols(qHydra_.third_.bipartite_graph_.array_matrix_,qHydra_.third_.bipartite_graph_.array_matrix_);
    RemoveEmptyRows(qHydra_.second_.bipartite_graph_.array_matrix_,qHydra_.second_.bipartite_graph_.array_matrix_);
    qHydra_.second_.AllScore();
    qHydra_.third_.AllScore();
    qHydra_.score_matrix_.array_matrix_ = af::matmul(qHydra_.third_.score_matrix_.array_matrix_,af::matmul(qHydra_.second_.score_matrix_.array_matrix_,qHydra_.first_.score_matrix_.array_matrix_.col(position)));
    vector<string> record;
    int flag = StringConcatenation(record,position,qHydra_.score_matrix_.array_matrix_);
    if(flag==1){
  ...
  virtual double Jackknife(float first_lambda,float second_lambda,float third_lamda){
    qHydra_.first_.SetLambda(first_lambda);
    qHydra_.second_.SetLambda(second_lambda);
    qHydra_.third_.SetLambda(third_lamda);
    qHydra_.first_.AllScore();
...
class qHydraBayesianOptimization: public bayesopt::ContinuousModel{
 public:
  qHydraCrossValidation qHydra_cv;
  void OutFileNameSetting(string set_out_file_name){
    qHydra_cv.SetFileName(set_out_file_name);
  }
  void BipartiteSetting(Matrix<int> &first_matrix,Matrix<int> &second_matrix,Matrix<int> &third_matrix,Matrix<int> &selection_matrix){
    for(int i = 0; i<selection_matrix.data_[0].size(); ++i){
      qHydra_cv.gene_selection_vector_.push_back(selection_matrix.data_[0][i]);
    }
    qHydra_cv.third_bipartite_graph_.data_ = third_matrix.data_;
    qHydra_cv.CreateDiseaseLink();
    qHydra_cv.qHydra_.first_.bipartite_graph_.TransformMatrixToArrayMatrix(first_matrix);
    qHydra_cv.qHydra_.second_.bipartite_graph_.TransformMatrixToArrayMatrix(second_matrix);
    qHydra_cv.qHydra_.third_.bipartite_graph_.TransformMatrixToArrayMatrix(third_matrix);
    qHydra_cv.Preserve();
  }
  void BipartiteSetting(Matrix<int> &first_matrix,Matrix<int> &second_matrix,Matrix<int> &third_matrix){
    qHydra_cv.Selection(third_matrix,second_matrix);
    qHydra_cv.third_bipartite_graph_.data_ = third_matrix.data_;
    qHydra_cv.CreateDiseaseLink();
    qHydra_cv.qHydra_.first_.bipartite_graph_.TransformMatrixToArrayMatrix(first_matrix);
    qHydra_cv.qHydra_.second_.bipartite_graph_.TransformMatrixToArrayMatrix(second_matrix);
  ...
  qHydraBayesianOptimization(bayesopt::Parameters param):
  ContinuousModel(3,param) {}
  double evaluateSample( const boost::numeric::ublas::vector<double> &query ){
    double value = qHydra_cv.Jackknife(query[0],query[1],query[2]);
    return (-1.0)*value;
  };
...
int main(){
  ...
  Matrix<int> first_bipartite("ExampleFirstBipartiteGraph.txt");
  Matrix<int> second_bipartite("ExampleSecondBipartiteGraph.txt");
  Matrix<int> third_bipartite("ExampleThirdBipartiteGraph.txt");
  Matrix<int> selection_matrix("ExampleIndex.txt");
  ...
  qHydraBayesianOptimization qHydrabo(parameters);
  ...
  qHydrabo.setBoundingBox(lower,upper);
  qHydrabo.BipartiteSetting(first_bipartite,second_bipartite,third_bipartite,selection_matrix);
  qHydrabo.OutFileNameSetting("qHydra(example)");
  qHydrabo.optimize(result);
